...
# Name:Zhe Cui, Yifai Alan Ng
# Email:cuizhefc@berkeley.edu, alanngyf@gmail.com
# login: cs61a-jw, cs61a-jz
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        curr_loc = prev_loc = self.place
        n, m, flag = min_range, 0, True
        while n > 0 :
            ...
            n -= 1
        if flag:
            while curr_loc != hive and len(curr_loc.bees) == 0 and m < max_range - min_range:
                prev_loc = curr_loc
                curr_loc = curr_loc.entrance
                m += 1
        if curr_loc == hive:
            return random_or_none(prev_loc.bees)
        return random_or_none(curr_loc.bees)
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            if len(self.place.bees) != 0:
                unlucky_bee = random.choice(self.place.bees)
                self.eat_bee(unlucky_bee)
...
class QueenPlace:
    ...
    def bees(self):
        "*** YOUR CODE HERE ***"
        return [bee for bee in self.colony_queen.bees] + [b for b in self.ant_queen.bees]
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            curr_loc = self.place
            prev_loc, next_loc = curr_loc.entrance, curr_loc.exit
            if curr_loc.ant.name == 'Bodyguard':
                if curr_loc.ant not in self.buffed_list:
                    curr_loc.ant.damage = curr_loc.ant.damage * 2
                    self.buffed_list.append(curr_loc.ant)
            while prev_loc != None and prev_loc is not colony.hive:
                if prev_loc.ant != None:
                    curr_ant = prev_loc.ant
                    if curr_ant not in self.buffed_list:
                        curr_ant.damage = curr_ant.damage * 2
                        self.buffed_list.append(curr_ant)
                    if curr_ant.name == 'Bodyguard':
                        if curr_ant.ant != None:
                            if curr_ant.ant not in self.buffed_list:
                                curr_ant.ant.damage = curr_ant.ant.damage * 2
                                self.buffed_list.append(curr_ant.ant)
                prev_loc = prev_loc.entrance
            while next_loc != None:
                if next_loc.ant != None:
                    curr_ant = next_loc.ant
                    if curr_ant not in self.buffed_list:
                        curr_ant.damage = curr_ant.damage * 2
                        self.buffed_list.append(curr_ant)
                    if curr_ant.name == 'Bodyguard':
...
def make_slow(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    def action2(self, colony):
        if colony.time % 2 == 0:
            print("even:", colony.time%2 == 0)
